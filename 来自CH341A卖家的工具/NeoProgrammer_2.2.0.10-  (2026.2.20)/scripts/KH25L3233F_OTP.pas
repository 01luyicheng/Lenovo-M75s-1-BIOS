...
begin
  if not SPIEnterProgMode(_SPI_SPEED_MAX) then LogPrint('Error setting SPI speed');
  LogPrint ('Start READ 4K-bit OTP page of KH25L3233F');
  RegSize :=32768;
  ...
  LogPrint ('End read OTP page of KH25L3233F');
  SPIExitProgMode ();
end
...
begin
  ...
  if (Confirm = 'YES') then
     begin
     LogPrint ('Start WRITE 4K-bit OTP page of KH25L3233F);
     SPIWrite (1, 1, $B1);                     // Enter OTP mode
     SPIWrite (0, 1, $2B);
     SPIRead  (1, 1, sreg);
     ...
     SPIWrite (1, 1, $C1);                      //Exit OTP mode
     LogPrint ('End write OTP page of KH25L3233F');
     end;
     SPIExitProgMode ();
end
