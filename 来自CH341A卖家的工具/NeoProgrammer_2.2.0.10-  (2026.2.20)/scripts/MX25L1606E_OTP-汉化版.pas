// designed for MACRONIX MX25L1606E
// READ / WRITE 64bit OTP PAGE, read and set OTP_LOCK bit
...
begin
  if not SPIEnterProgMode(_SPI_SPEED_MAX) then LogPrint('Error setting SPI speed');
  LogPrint ('Start READ 64bit OTP page of MX25L1606E');
  RegSize :=64;
  ...
  LogPrint ('End read OTP page of MX25L1606E');
  SPIExitProgMode ();
end
...
begin
  ...
  if (Confirm = 'YES') then
     begin
     LogPrint ('Start WRITE 64bit OTP page of MX25L1606E);
     SPIWrite (1, 1, $B1);                     // Enter OTP mode
     SPIWrite (0, 1, $2B);
     SPIRead  (1, 1, sreg);
     ...
     SPIWrite (1, 1, $C1);                      //Exit OTP mode
     LogPrint ('End write OTP page of MX25L1606E');
     end;
     SPIExitProgMode ();
end
